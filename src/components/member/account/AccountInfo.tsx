...
export interface MemberInfo {
    ...
    // --- thêm các field profile để edit ---
    phoneNumber: string;
    dob: string;          // yyyy-mm-dd
    address: string;      // địa chỉ dòng 1 / street
    city: string;         // province name
    state: string;        // district name
    zip: string;
    country: string;
    district?: string;   // NEW
    ward?: string;     // e.g., Vietnam / USA
}
...
export function AccountInfo({
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                // match theo tên nếu country là Vietnam
                if ((memberInfo.country || "").toLowerCase().includes("viet")) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const dMatch = ds.find(d => d.name.toLowerCase() === (memberInfo.state || "").toLowerCase());
    ...
    async function onSave() {
        ...
        const cityName = selectedProvince?.name || memberInfo.city || "";
        const stateName = selectedDistrict?.name || memberInfo.state || "";
        const wardName = selectedWard?.name || memberInfo.ward || "";
        const districtName = selectedDistrict?.name || memberInfo.district || "";
        const payload: any = {};

        // chỉ đưa field thay đổi
        ...
        try {
            ...
            // cập nhật local state cho khớp UI
            ...
            // ghi đè trở lại props internal (ở thực tế props đến từ page, nhưng ta có thể dùng callback / lift state;
            // ở đây đơn giản set lại các state form và đóng edit)
            setIsEditing(false);
            toast({ title: t("common.saved") || "Saved" });
        } catch (e: any) {
            toast({ title: t("common.error") || "Error", description: e?.message || "Update failed", variant: "destructive" });
        } finally {
            setSaving(false);
        }
    }
            ...
            <Card className="border-teal-200">
                ...
                <CardContent>
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        {isEditing && country.toLowerCase().includes("viet") && (
                            <>
                                <div className="space-y-2">
